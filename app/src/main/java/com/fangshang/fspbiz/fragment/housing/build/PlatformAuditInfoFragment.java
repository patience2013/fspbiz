...
public class PlatformAuditInfoFragment extends BaseFragment {
    ...
    protected void initView(Bundle savedInstanceState) {
        if(null!=buildDetail){
            //提交时间
            mTv_committime.setText(ZhuanHuanUtil.getTimes(buildDetail.estate.createTime+""));
            //审核状态
            if(buildDetail.estate.approveSta==0){
                mTv_audit_status.setText("未审核");
            }else if(buildDetail.estate.approveSta==1){
            ...
            }else if(buildDetail.estate.approveSta==2){
                mTv_audit_status.setText("审核中");
                mLin_nopassreason.setVisibility(View.GONE);
                mLin_update_time.setVisibility(View.GONE);
                mLin_cuishen.setVisibility(View.VISIBLE);
            }else if(buildDetail.estate.approveSta==3){
            ...
            //审核时间
            mTv_audit_time.setText(null==buildDetail.estate.approveTime? "无":ZhuanHuanUtil.getTimes(buildDetail.estate.approveTime));
            //失败原因
            mTv_issue.setText(null==buildDetail.estate.approveIssue?"无":buildDetail.estate.approveIssue);
            //修改时间
            mTv_update_time.setText(null==buildDetail.estate.updateTime?"无修改记录":ZhuanHuanUtil.getTimes(buildDetail.estate.updateTime+""));
            //催审时间
            mTv_cuishen_time.setText(null==buildDetail.estate.updateTime?"催审时间没字段":"催审时间没字段");
        }
    }
}
