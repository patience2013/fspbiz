...
public class AddHouseActivity extends BaseActivity{
    ...
    public static void actionStart(Context context,String buildId,String buildName,String houseId) {
...
//                    progressBarSum.setVisibility(View.GONE);
        ...
        mWeb_add_build.loadUrl(App.H5_HOST+"newListings.html?create");
        try {
    ...
    public void sendInfoToJs() {
        ...
        //列表新增房源
        if("".equals(buildId) && "".equals(houseId) && "".equals(buildName)){
            mWeb_add_build.loadUrl("javascript:getToken('" + AccountHelper.getUser().getToken() +","+AccountHelper.getUser().getBussType()+"')");
        }else if(!"".equals(buildId) && "".equals(houseId)){//列表item新增房源
            mWeb_add_build.loadUrl("javascript:getToken('" + AccountHelper.getUser().getToken() +","+AccountHelper.getUser().getBussType()+","+buildName+","+buildId+"')");
        }else if(!"".equals(buildId) && !"".equals(houseId)){ //修改房源
            mWeb_add_build.loadUrl("javascript:getToken('" + AccountHelper.getUser().getToken() +","+AccountHelper.getUser().getBussType()+","+buildName+","+buildId+","+houseId+"')");
        }
    }
...
//        TsUtils.show(path);
    ...
    public void initAdapter(){
        //新的图片选择器
        FullyGridLayoutManager manager = new FullyGridLayoutManager(AddHouseActivity.this, 4, GridLayoutManager.VERTICAL, false);
        recyclerView.setLayoutManager(manager);
        adapter = new GridVideoAdapter(AddHouseActivity.this, onAddPicClickListener);
                ...
                if (selectList.size() > 0) {
                    ...
                    switch (mediaType) {
                        case 1:
                            ...
                            PictureSelector.create(AddHouseActivity.this).externalPicturePreview(position, selectList);
                            break;
                        case 2:
                            // 预览视频
                            PictureSelector.create(AddHouseActivity.this).externalPictureVideo(media.getPath());
                            break;
                        case 3:
                            // 预览音频
                            PictureSelector.create(AddHouseActivity.this).externalPictureAudio(media.getPath());
                            break;
                    }
                }
            }
    ...
    @OnClick({R.id.img_addvideo,R.id.lin_back,R.id.tv_addbuild})
    public void onClick(View view){
        switch (view.getId()){
                ...
                if (selectList.size() > 0) {
                    ...
                    switch (mediaType) {
                        case 1:
                            // 预览图片 可自定长按保存路径
                            //PictureSelector.create(MainActivity.this).externalPicturePreview(position, "/custom_file", selectList);
                            PictureSelector.create(AddHouseActivity.this).externalPicturePreview(0, selectList);
                            break;
                        case 2:
                            // 预览视频
                            PictureSelector.create(AddHouseActivity.this).externalPictureVideo(media.getPath());
                    ...
                    PictureSelector.create(AddHouseActivity.this)
            ...
            case R.id.lin_back:
                ...
                break;
            case R.id.tv_addbuild:
                AddBuildActivity.actionStart(AddHouseActivity.this,"");
                break;
        }
    }
...
//                mLin_progress.setVisibility(View.GONE);
    ...
    public void yasuoImage(File file){
        Luban.with(AddHouseActivity.this)
